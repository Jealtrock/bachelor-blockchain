...
Bei einem erfolgreichen Angriff würde die eigene Wallet nicht mit der Zahlung an den Verkäufer belastet werden. Ein weiterer Nebeneffekt ist, das womöglich viele Transaktionen in der Hauptkette nicht mehr validiert sind und erneut aufgenommen werden müssen.
...
Das Objekt wird auf Syntax und Gültigkeit geprüft. Dafür wird die Transaction-Hash abgeglichen und die Signierung überprüft. Wenn die Transaktionen gültig sind, kann die eigene Transaktion diese referenzieren und publiziert werden.
...
Eine falsche Transaktion (zum Beispiel nicht genügend IOTAs) kann so also von entsprechenden Tips in einer Sub-Tangle vorerst validiert werden, jedoch wird spätestens wenn andere Transaktionen die Sub-Tangle mit der anderen Sub-Tangle validieren, entsteht ein Konflikt. Der Konflikt wird aufgelöst indem neue Tips ausgewählt werden und so eine der beiden Sub-Tangles nicht weiter verifiziert wird. Dadurch verwaist eine Sub-Tangle und diejenige mit dem höheren "`Gewicht"' (mehr Proof-Of-Work) wird für neue Transaktionen genutzt. Das heißt auch, das die anderen, validen Transaktionen, die die Fehlerhafte Transaktion referenzierten, nun auch verwaisen und nicht mehr Bestätigt werden können. Diese müssen neu in die Tangle attached werden um eine weitere Chance auf Validierung zu bekommen.
